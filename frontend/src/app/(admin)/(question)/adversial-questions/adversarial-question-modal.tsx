...
export default function AdversarialQuestionModal({
  ...
  mode,
  question_id,
}: Readonly<AdversarialQuestionModalProps>) {
  ...
  const [generated, setGenerated] =
    useState<GeneratedAdversarialQuestion | null>(null);
    ...
    try {
      ...
      setGenerateError(
        err instanceof Error
          ? err.message
          : "Failed to generate adversarial question.",
      );
    } finally {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      setRegenerateError(
        err instanceof Error
          ? err.message
          : "Failed to regenerate adversarial question.",
      );
    } finally {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      setValidationError(
        err instanceof Error
          ? err.message
          : "Failed to validate adversarial question.",
      );
    } finally {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      setDeployError(
        err instanceof Error
          ? err.message
          : "Failed to save adversarial question.",
      );
    } finally {
      ...
      <div className="bg-secondary-surface border border-tertiary-surface rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-tertiary-surface flex items-center justify-between bg-secondary-surface">
          ...
          <button
            ...
            onClick={onClose}
            className="text-default-border hover:text-system-red"
          >
            <X size={24} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            id="source-question"
              ...
              onChange={(e) => {
                setSourceQuestionId(Number(e.target.value));
          ...
          <div>
            <label 
                htmlFor="adversarial-technique"
                className="block text-sm font-medium mb-2">
              Adversarial Technique
            </label>
            <select
                id="adversarial-technique"
              value={strategyId ?? ""}
              onChange={(e) => {
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={
                !generated || !strategyId || isRegenerating || isGenerating
              }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="border border-tertiary-surface rounded p-5 bg-secondary-surface">
              <h3 className="font-staatliches text-lg mb-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="border border-tertiary-surface rounded p-5 bg-secondary-surface">
              <h3 className="font-staatliches text-lg mb-3">
                Gemini Response to Adversarial Question
              </h3>
          ...
          {validationError && (
            <p className="text-xs text-system-red">{validationError}</p>
          )}
          {deployError && (
            <p className="text-xs text-system-red">{deployError}</p>
          )}
          ...
          {selectedSource && (
            <div className="text-xs text-white-smoke/60 mt-2">
              Source Category:{" "}
              {categories.find(
                (c) => c.category_id === selectedSource.category_id,
