...
function mapCandidateAssessment(
  session: CandidateAssessmentApi,
): AssessmentCardProps | null {
...
function AssessmentPageContent() {
  ...
  const filteredAssessments = useMemo(() => {
    return assessments.filter((assessment) => {
      ...
      const descMatches = assessment.description
        ?.toLowerCase()
  ...
  return (
    <main className="min-h-screen px-8 py-8">
      <div className="mt-8 flex items-start justify-between">
        <div>
          <h1 className="text-3xl text-default-text mb-2">
      ...
      {isLoading ? (
        ...
        <div className="pt-8 text-white-smoke">No assessments assigned yet</div>
      ) : (
...
export default function AssessmentPage() {
  return (
    <main>
      <Suspense
        fallback={<div className="pt-8 text-default-text">Loading View...</div>}
      >
