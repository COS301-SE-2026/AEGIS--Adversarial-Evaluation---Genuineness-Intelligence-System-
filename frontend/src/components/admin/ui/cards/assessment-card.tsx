            ...
            {MOCK_CANDIDATES.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {isAssigned ? (
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#66BB6A" strokeWidth="2.5">
                      <polyline points="20 6 9 17 4 12"/>
                    </svg>
                  ) : (
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="rgba(245,245,245,0.28)" strokeWidth="1.8">
...
export default function AssessmentCard({ assessment: a }: AssessmentCardProps) {
  ...
  return (
    <div className="bg-[#292C2F] border border-[#333331] rounded-[5px] px-5 py-[18px] cursor-pointer relative overflow-hidden transition-all duration-150 hover:border-[rgba(211,47,47,0.4)] hover:bg-[rgba(41,44,47,0.85)] group">
      <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-[5px]" style={{ background: accentColour[a.status] }} />
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {a.types.map((t) => (
          <span key={t} className="font-jetbrains text-[9px] px-2 py-0.5 bg-[#222426] border border-[#333331] rounded-[5px] text-[rgba(245,245,245,0.22)] tracking-[0.04em]">
            {t}
          </span>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-2 border-t border-[#333331] pt-3">
        <div className="text-center">
          <div className="font-staatliches text-[20px] tracking-[0.02em] leading-none text-[#F5F5F5]">{a.candidates}</div>
          <div className="font-jetbrains text-[8px] text-[rgba(245,245,245,0.42)] tracking-[0.06em] uppercase mt-0.5">Candidates</div>
        </div>
        <div className="text-center">
          <div className="font-staatliches text-[20px] tracking-[0.02em] leading-none text-[#F5F5F5]">{completionPct}%</div>
          <div className="font-jetbrains text-[8px] text-[rgba(245,245,245,0.42)] tracking-[0.06em] uppercase mt-0.5">Completed</div>
        </div>
        <div className="text-center">
          <div className={`font-staatliches text-[20px] tracking-[0.02em] leading-none ${aiHighlight ? "text-[#EF5350]" : "text-[#F5F5F5]"}`}>
            {a.aiRate > 0 ? `${a.aiRate}%` : "—"}
          </div>
          <div className="font-jetbrains text-[8px] text-[rgba(245,245,245,0.42)] tracking-[0.06em] uppercase mt-0.5">AI Detect</div>
        </div>
      </div>
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#333331]">
        <div className="font-jetbrains text-[10px] text-[rgba(245,245,245,0.42)]">{a.created}</div>
