...
export default function QuestionTable({
  ...
  const difficultyStyle = (difficulty: string) => {
    switch (difficulty.toLowerCase()) {
      ...
      default:
        return "bg-system-red/20 text-system-red";
    }
  };
          ...
          <thead>
            <tr className="border-b border-default-border">
              <th
                ...
                className="px-4 sm:px-6 py-3 sm:py-4 text-left text-default-text font-semibold text-xs sm:text-sm cursor-pointer hover:text-system-red transistion-colors"
              >
              ...
              <th
                ...
                className="hidden lg:table-cell px-4 sm:px-6 py-3 sm:py-4 text-left text-default-text font-semibold text-xs sm:text-sm cursor-pointer hover:text-system-red transition-colors"
              >
                <div className="flex items-center gap-2">
                  Category{" "}
                  {sortColumn === "category" &&
              ...
              <th
                ...
                className="px-4 sm:px-6 py-4 text-left text-default-text font-semibold text-xs sm:text-sm cursor-pointer hover:text-system-red"
              >
                <div className="flex items-center gap-2">
                  Difficulty{" "}
          ...
          <tbody>
            {questions.map((question) => (
              ...
              >
                ...
                <td className="hidden md:table-cell px-4 sm:px-6 py-3 sm:py-4 text-default-text text-xs sm:text-sm max-w-xs truncate">
                  {question.content}
                </td>
                ...
                <td className="px-4 sm:px-6 py-3 sm:py-4 text-center">
                  <span
                    className={`px-2 py-1 rounded text-xs font-medium ${difficultyStyle(question.difficulty)}`}
                  >
                    {question.difficulty}
                  </span>
                </td>
                <td className="px-4 sm:px-6 py-3 sm:py-4 text-center relative">
                  ...
                  {openMenuId === question.question_bank_id && (
                    <div className="absolute right-full top-1/2 -translate-y-1/2 px-4 py-2 mr-2 w-28 bg-secondary-surface border border-default-border rounded shadow-lg z-10">
                      <button
                        type="button"
                        onClick={() => onEdit(question.question_bank_id)}
                        className="block w-full text-left px-4 py-2 text-sm text-default-text hover:bg-tertiary-surface transition-colors"
                      >
                        Edit
                      </button>
                      <button
                        type="button"
                        onClick={() => onDelete(question.question_bank_id)}
                        className="block w-full text-left px-4 py-2 text-sm text-system-red hover:bg-tertiary-surface transition-colors border-t border-default-border"
                      >
                        Delete
                      </button>
                    </div>
