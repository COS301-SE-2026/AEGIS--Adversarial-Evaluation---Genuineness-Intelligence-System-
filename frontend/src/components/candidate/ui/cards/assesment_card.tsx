...
export function AssessmentCard({ assessmentId, title, description, num_questions, attempts, success_rate }: AssessmentCardProps) {
    return (
        <div className= "bg-secondary-surface border-2 border-tertiary-surface p-4 h-20rem w-15rem flex flex-col hover:scale-105 hover:shadow-md hover:shadow-tertiary-surface/50 transition-all duration-300">
            ...
            <div className="mt-auto">
                <Link href={`/assessment/${assessmentId}`}>
                    <button className= " mt-4 bg-transparent h-3rem w-8rem text-system-red border-2 border-system-red  px-4 py-2 hover:bg-system-red hover:text-white-smoke transition-colors duration-300 cursor-pointer">
