...
export function AssessmentPreviewModal({assessment, onClose,onStart}: AssessmentPreviewModalProps) {
    // mounting a component is used to make the modal show on screen for the first time this prevents server side rendering from Next.js
    // the useEffect will then change this modal after it has mounted.
    const [mounted, setMounted] = useState(false);

    useEffect(() => {
        setMounted(true);
        
        // disable background page scrolling whilst the modal is open
    ...
    if(!mounted) return null; //pre

    return createPortal (
        <div className="fixed inset-0 z-9999 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 sm:p-6"
            onClick={onClose}
        >
            <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg border-2 border-default-border/70 bg-background p-6 sm:p-8 shadow-2xl"
            ...
            <div className="flex-1 overflow-auto-y pr-2 space-y-6 mb-6 min-h-0">
                ...
                <div className="flex items-center border border-status-success/80 rounded-md p-4 w-36">
                    <div className="flex items-center flex-1">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-3 text-default-text mb-5">
                ...
                <div className="space-y-3">
                    <h4 className="text-xs uppercase tracking-wider foont-medium text-default-text">
                        Test Rules &amp; Instructions
                    </h4>
                    <ul className="space-y-3">
                        {TestRules.map((rule, index) => (
                            <li key={index} className="flex items-start text-sm text-default-text leading-relaxed">
                ...
                <div className="shrink-0 border-t border-default-border mt-4 pt-4">
                    <span className="text-sm text-default-text italic mb-4 leading-normal">
                        By clicking "Start", you acknowledge that you have read, understood, and agree to abide by these terms and conditions
                    </span>
